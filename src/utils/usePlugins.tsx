...
/**
 * Renders every plugin element registered for a given position. Renders
 * nothing when no plugin targets that slot, so it is safe to drop into any
 * layout (header, body, footer or the options page).
 */
export const PluginSlot = ({ position }: { position: PluginPosition }): ReactElement => {
  const ctx = usePluginsContext();
  const elements = ctx[position];
